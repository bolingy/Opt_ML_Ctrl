...
class agent():
    def __init__(self, lr, s_size, a_size, h_size, scope_name):
        ...
        with tf.name_scope('some_scope1'):
            ...
            hidden_0 = tf.layers.dense(self.state_in, h_size, bias_initializer=None,activation=tf.nn.relu)

            hidden_1 = tf.layers.dense(hidden_0, h_size, bias_initializer=None,activation=tf.nn.relu)

            #hidden_2 = tf.layers.dense(hidden_1, h_size, bias_initializer=None, activation=tf.nn.relu)

            self.output = tf.layers.dense(hidden_1, a_size,activation=tf.nn.softmax,\
...
total_episodes = 5000  #5000 Set total number of episodes to train agent on.
max_ep = 999
update_frequency = 5
...
if __name__ == '__main__':
    a = agent(lr=1e-2, s_size=4, a_size=2, h_size=8)  # Load the agent.